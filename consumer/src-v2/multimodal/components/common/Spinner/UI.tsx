...
import { createComponent } from '../../../../../src/typescript/utils/createComponent';
import { styleAdapter } from '../../../../../src/typescript/utils/styleAdapter';
...
export type SpinnerTheme = 'base' | 'primary' | 'secondary' | 'success' | 'danger' | 'default';
...
const RNSpinner = forwardRef<Animated.View, Partial<SpinnerProps>>((props, ref) => {
    const { style: ts, color: gc } = tailwind;

    const { size = 'md', track = 'transparent', themeColor = 'base', stroke, style } = props;
    ...
    const spinnerStroke = stroke ? gc(stroke) : gc(spinnerTheme.themeColor[themeColor]);
    ...
    const indeterminateAnimatedCircularProgress = useAnimatedProps(() => {
        return {
            strokeDashoffset: interpolate(progress.value, [0, 0.5, 1], [0, -276, -(276 * 2)]),
        };
    });
    ...
    return (
        <Animated.View
            ref={ref}
            style={[ts(spinnerTheme.size[size]), styleAdapter(style, undefined), animatedSvgStyle]}>
            <Svg width="100%" height="100%" viewBox={'0 0 100 100'}>
                ...
                <G rotation={'-90'} origin="50, 50">
                    <Circle
                        stroke={gc(
                            track === 'transparent' ? spinnerTheme.track[track] : spinnerTheme.track[track][themeColor],
                        )}
                        strokeWidth={10}
                        fill={'transparent'}
                        r={radius}
