...
let decodeOneWayBookingAPIDetails = data => {
    ...
    Ok(
      ...
      ->(
        dict => {
          ...
          isUpgradedToCab: getOptionBool(dict, "isUpgradedToCab"),
          stops: dict
          ->Dict.get("stops")
  ...
  | err => {
      Console.log2("OneWayBookingAPIDetails ERROR", err)
      Error(err)
    }
  }
}

let toJson = (req: oneWayBookingAPIDetails) => {
  req->asJson
}
