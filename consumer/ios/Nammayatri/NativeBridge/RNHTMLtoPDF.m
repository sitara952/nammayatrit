...
-(void)webView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation {
    ...
    if (pdfData) {
        ...
        [topVC presentViewController:activityController animated:YES completion:nil];
        _resolveBlock(data);
    } else {
        NSError *error;
...
RCT_EXPORT_METHOD(downloadPdfFromUrl:(NSString *)certificateUrl
                  resolver:(RCTPromiseResolveBlock)resolve
                  rejecter:(RCTPromiseRejectBlock)reject) {
    NSURL *sourceURL = [NSURL URLWithString:certificateUrl];
    if (!sourceURL) {
        reject(@"invalid_url", @"The certificate URL is invalid.", nil);
        return;
    }

    NSURLSession *session = [NSURLSession sharedSession];
    NSURLSessionDownloadTask *downloadTask = [session downloadTaskWithURL:sourceURL completionHandler:^(NSURL *location, NSURLResponse *response, NSError *error) {
        if (error || !location) {
            reject(@"download_error", @"Failed to download certificate.", error);
            return;
        }
        ...
        if (!fileName || [fileName length] == 0 || ![fileName containsString:@".pdf"]) {
        ...
        if (![[NSFileManager defaultManager] moveItemAtURL:location toURL:tempURL error:&fileError]) {
            reject(@"move_file_error", @"Failed to move downloaded file to temp location.", fileError);
            return;
        }

        dispatch_async(dispatch_get_main_queue(), ^{
            UIActivityViewController *activityController = [[UIActivityViewController alloc] initWithActivityItems:@[tempURL] applicationActivities:nil];

            UIViewController *topVC = [self topViewController];
            if (!topVC) {
                reject(@"no_top_view_controller", @"Unable to find top view controller.", nil);
                return;
            }
            ...
            activityController.completionWithItemsHandler = ^(UIActivityType __nullable activityType, BOOL completed, NSArray * __nullable returnedItems, NSError * __nullable activityError) {
                if (completed) {
                    ...
                    reject(@"activity_error", @"Error during sharing activity.", activityError);
                } else {
                    reject(@"cancelled", @"User cancelled sharing.", nil);
                }
            };
        });
    }];
