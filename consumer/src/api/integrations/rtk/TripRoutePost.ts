...
export type tripRoutePostWithParams = {
    body: getRoutesReq;
};
...
export const apiCall = (rtkExtraOptions: RtkExtraOptions) =>
    api.injectEndpoints({
        endpoints: build => ({
            tripRoutePost: build.mutation<routeInfoArray, tripRoutePostWithParams>({
                query: ({ body }) => ({
                    url: (function () {
                        const url = '/trip/route' + '?';
                        return url;
                    })(),
                    method: 'POST',
                    body: body,
...
export const { useTripRoutePostMutation } = apiCall({ onQueryStarted: undefined, keepUnusedDataFor: undefined });
//
